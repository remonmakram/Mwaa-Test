...
BEGIN
   EXECUTE IMMEDIATE 'DROP TABLE RAX_APP_USER.REPORT_RUN_DATE_TEMP';
EXCEPTION
   WHEN OTHERS THEN
      IF SQLCODE != -942 THEN
         RAISE;
...
BEGIN
   EXECUTE IMMEDIATE 'DROP TABLE RAX_APP_USER.SHIPMENTS_BY_EVENT_STG';
EXCEPTION
   WHEN OTHERS THEN
      IF SQLCODE != -942 THEN
         RAISE;
...
insert into RAX_APP_USER.SHIPMENTS_BY_EVENT_STG
(plant_name,calendar_year,calendar_week_number,job_nbr,sales_line_name,program_name,sub_program_name,account_name,trans_date,shipped_order_qty)
values
('plant_name',2024,10,0,'sales_line_name','program_name','sub_program_name','account_name',sysdate,0)
...
BEGIN
   EXECUTE IMMEDIATE 'DROP TABLE RAX_APP_USER.RECORD_COUNT_TEMP';
EXCEPTION
   WHEN OTHERS THEN
      IF SQLCODE != -942 THEN
         RAISE;
