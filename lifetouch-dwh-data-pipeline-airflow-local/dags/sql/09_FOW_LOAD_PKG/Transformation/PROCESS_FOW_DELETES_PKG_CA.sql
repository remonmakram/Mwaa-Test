...
MERGE INTO ODS_OWN.APO a
   USING (SELECT xr.apo_oid
            FROM  ODS_STAGE_CA.FOW_RECORD_CHANGE_AUDIT aud,
...
/*-----------------------------------------------*/
/* TASK No. 5 */
/* delete from ODS_STAGE_CA.FOW_APO_STAGE */

delete from ODS_STAGE_CA.FOW_APO_STAGE z where z.APO_OID in ( 
select 
    apo_xr.APO_OID
from 
    ODS_STAGE_CA.FOW_RECORD_CHANGE_AUDIT rca 
    ,ODS_STAGE_CA.APO_XR apo_xr
where (1=1)
...
/* TASK No. 6 */
/* delete 
from ODS_STAGE_CA.FOW_EVENT_STG */
...
/*-----------------------------------------------*/
/* TASK No. 7 */
/* Update ODS_STAGE_CA.FOW_EVENT_STG  */
...
/*-----------------------------------------------*/
/* TASK No. 11 */
/* delete  from ODS_STAGE_CA.EVENT_XR */

delete 
from ODS_STAGE_CA.EVENT_XR z where z.EVENT_OID in ( 
select 
    e_xr.EVENT_OID
from 
    ODS_STAGE_CA.FOW_RECORD_CHANGE_AUDIT rca 
...
/* TASK No. 12 */
/* delete 
from ODS_STAGE_CA.FOW_OFFERING_STG */

delete 
from ODS_STAGE_CA.FOW_OFFERING_STG z where z.ID in ( 
select 
    rca.KEY_VALUE ID
--    o_xr.ID
from 
    ODS_STAGE_CA.FOW_RECORD_CHANGE_AUDIT rca 
--    ,ODS_STAGE_CA.FOW_OFFERING_XR o_xr
where (1=1)
...
/* TASK No. 14 */
/* delete 
from ODS_STAGE_CA.FOW_OFFERING_XR */

delete 
from ODS_STAGE_CA.FOW_OFFERING_XR z where z.OFFERING_OID in ( 
select 
    o_xr.OFFERING_OID
-- select *
from 
...
/* TASK No. 15 */
/* delete 
from ODS_STAGE_CA.FOW_AUTO_ORDER_LINE_STG */

delete 
from ODS_STAGE_CA.FOW_AUTO_ORDER_LINE_STG z where z.ID in ( 
select 
    rca.KEY_VALUE ID
from 
    ODS_STAGE_CA.FOW_RECORD_CHANGE_AUDIT rca 
...
from 
    ODS_STAGE_CA.FOW_RECORD_CHANGE_AUDIT rca 
    ,ODS_STAGE_CA.FOW_AUTO_ORDER_LINE_XR o_xr
where (1=1)
    and rca.TABLE_NAME = 'AUTO_ORDER_LINE'
    and rca.EVENT_TYPE = 'DELETE'
    and rca.KEY_VALUE = o_xr.ID
...
/* TASK No. 17 */
/* delete 
from ODS_STAGE_CA.FOW_AUTO_ORDER_LINE_XR */

delete 
from ODS_STAGE_CA.FOW_AUTO_ORDER_LINE_XR z where z.AUTO_ORDER_LINE_OID in ( 
select 
    o_xr.AUTO_ORDER_LINE_OID
-- select *
from 
...
/*-----------------------------------------------*/
/* TASK No. 18 */
/* delete  from ODS_STAGE_CA.fow_apo_merch_tags */

delete 
from ODS_STAGE_CA.fow_apo_merch_tags z where z.ID in ( 
select 
    rca.key_value id
from 
    ODS_STAGE_CA.fow_record_change_audit rca 
...
from 
    ODS_STAGE_CA.FOW_RECORD_CHANGE_AUDIT rca 
    , ODS_STAGE_CA.FOW_APO_TAG_XR  xr
where (1=1)
    and rca.TABLE_NAME = 'APO_MERCH_TAGS'
    and rca.EVENT_TYPE = 'DELETE'
...
/*-----------------------------------------------*/
/* TASK No. 20 */
/* delete from ODS_STAGE_CA.apo_tag_xr */

delete 
from ODS_STAGE_CA.FOW_APO_TAG_XR  z where z.apo_tag_oid in ( 
select 
    xr.apo_tag_oid
from 
    ODS_STAGE_CA.FOW_RECORD_CHANGE_AUDIT rca 
...
/*-----------------------------------------------*/
/* TASK No. 21 */
/* delete from ODS_STAGE_CA.fow_yb_offering_config_stg */

delete 
from ODS_STAGE_CA.fow_yb_offering_config_stg z where z.id in ( 
select 
    rca.key_value id
from 
    ODS_STAGE_CA.fow_record_change_audit rca 
...
from 
    ODS_STAGE_CA.fow_record_change_audit rca 
    , ODS_STAGE_CA.yb_offering_config_xr   xr
where (1=1)
    and rca.table_name = 'YB_OFFERING_CONFIG'
    and rca.event_type = 'DELETE'
    and rca.key_value = xr.ID
...
/*-----------------------------------------------*/
/* TASK No. 23 */
/* delete from ODS_STAGE_CA.yb_offering_config_xr  */

delete 
from ODS_STAGE_CA.yb_offering_config_xr   z where z.yb_offering_config_oid in ( 
select 
    xr.yb_offering_config_oid
from 
    ODS_STAGE_CA.fow_record_change_audit rca 
...
/*-----------------------------------------------*/
/* TASK No. 24 */
/* delete from ODS_STAGE_CA.fow_yb_flow_stg */

delete 
from ODS_STAGE_CA.fow_yb_flow_stg z where z.flow_id in ( 
select 
    rca.key_value id
from 
    ODS_STAGE_CA.fow_record_change_audit rca 
...
/*-----------------------------------------------*/
/* TASK No. 25 */
/* delete from ODS_STAGE_CA.fow_yb_flow_booking_year_stg */

delete 
from ODS_STAGE_CA.fow_yb_flow_booking_year_stg z where (z.flow_id, z.booking_year) in ( 
select 
...
/*-----------------------------------------------*/
/* TASK No. 26 */
/* delete from ODS_STAGE_CA.fow_yb_flow_feature_stg */

delete 
from ODS_STAGE_CA.fow_yb_flow_feature_stg z where (z.flow_id, z.feature_id) in ( 
select 
    rca.key_value, rca.key_value2
from 
    ODS_STAGE_CA.fow_record_change_audit rca 
...
/*-----------------------------------------------*/
/* TASK No. 27 */
/* delete from ODS_STAGE_CA.fow_yb_feature_stg */

delete 
from ODS_STAGE_CA.fow_yb_feature_stg z where z.feature_id in ( 
select 
    rca.key_value id
from 
    ODS_STAGE_CA.fow_record_change_audit rca 
...
/*-----------------------------------------------*/
/* TASK No. 28 */
/* delete from ODS_STAGE_CA.fow_yb_feature_choice_stg */

delete 
from ODS_STAGE_CA.fow_yb_feature_choice_stg z where (z.feature_id, z.choice_id) in ( 
select 
    rca.key_value, rca.key_value2
from 
    ODS_STAGE_CA.fow_record_change_audit rca 
...
/*-----------------------------------------------*/
/* TASK No. 29 */
/* delete from ODS_STAGE_CA.fow_yb_choice_stg */

delete 
from ODS_STAGE_CA.fow_yb_choice_stg z where z.choice_id in ( 
select 
    rca.key_value id
from 
    ODS_STAGE_CA.fow_record_change_audit rca 
...
delete 
from ODS_STAGE_CA_ca.FOW_EVENT_STG z where z.ID in ( 
select 
    e_xr.FOW_CA_ID
from 
    ODS_STAGE_CA_ca.FOW_RECORD_CHANGE_AUDIT rca 
    ,ODS_STAGE_CA.EVENT_XR e_xr
where (1=1)
    and rca.TABLE_NAME = 'EVENT'
    and rca.EVENT_TYPE = 'DELETE'
...
UPDATE ODS_STAGE_CA_ca.FOW_EVENT_STG z
   SET status = 'DELETED'
 WHERE     z.ID IN
               (SELECT e_xr.FOW_CA_ID
                  FROM ODS_STAGE_CA_ca.FOW_RECORD_CHANGE_AUDIT  rca,
                       ODS_STAGE_CA.EVENT_XR                 e_xr
                 WHERE     (1 = 1)
                       AND rca.TABLE_NAME = 'EVENT'
                       AND rca.EVENT_TYPE = 'DELETE'
