...
where NOT EXISTS 
	( select 1 from ODS_STAGE_CA.FOW_EVENT_STG T
	where	T.ID	= S.ID 
...
/* DETECTION_STRATEGY = NOT_EXISTS */
update	ODS_STAGE_CA.FOW_EVENT_STG T
set 	
	(
	T.VERSION,
	T.ACTIVE,
...
/* DETECTION_STRATEGY = NOT_EXISTS */
insert into 	ODS_STAGE_CA.FOW_EVENT_STG T
	(
	ID,
	VERSION,
	ACTIVE,
...
DELETE FROM ODS_STAGE_CA.FOW_EVENT_STG FES
      WHERE EXISTS
               (SELECT 1
                  FROM ODS_STAGE_CA.IGNORE_FOW_CA_EVENTS_SODS_1178 ICFE
...
SELECT 
ODS_STAGE_CA.EVENT_TYPE_OID_SEQ.nextval
, STG.EVENT_TYPE
, STG.EVENT_TYPE
...
select e.status
, e.ods_modify_date
from ODS_STAGE_CA.FOW_EVENT_STG se
...
merge into ODS_STAGE_CA.EVENT_XR t
using
(
...
from ODS_STAGE_CA.FOW_EVENT_STG se
, ODS_STAGE_CA.FOW_APO_STAGE sapo
, ODS_OWN.EVENT e
...
values
( nvl(s.event_oid, ODS_STAGE_CA.event_oid_seq.nextval)
, s.event_ref_id
, s.id
, s.event_type
, s.apo_tag
...
, MART.marketing m3
, MART.marketing m4
, ODS_STAGE_CA.APO_APOCS_EVENT_XR aexr -- hardcoded schema so this works in PROD_CA context, other option would be to create a view in ODS_STAGE_CA_CA schema
, ODS_STAGE_CA.APO_XR axr
, ODS_OWN.APO apo2
where d.id = se.id
