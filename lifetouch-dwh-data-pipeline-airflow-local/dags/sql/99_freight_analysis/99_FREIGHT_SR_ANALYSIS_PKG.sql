...
truncate table rax_app_user.freight_driver_sr
...
INSERT INTO freight_driver_sr
    SELECT DISTINCT s.shipment_oid
      FROM ods_own.source_system ss,
           ods_own.shipment       s
     WHERE s.SOURCE_SYSTEM_OID = ss.SOURCE_SYSTEM_OID
...
INSERT INTO freight_driver_sr dr
    SELECT DISTINCT sl.shipment_oid
     ...
     WHERE     sl.SOURCE_SYSTEM_OID = ss.SOURCE_SYSTEM_OID
           ...
           AND ss.SOURCE_SYSTEM_SHORT_NAME = 'OMS2'
           AND sl.ods_modify_date >
                 TO_DATE (
                     SUBSTR (:v_cdc_load_date, 1, 19),
...
INSERT INTO freight_driver_sr dr
    SELECT DISTINCT sc.shipment_oid
      FROM ods_own.shipment_container  sc,
           ODS_OWN.SHIPMENT            s,
           ods_own.source_system       ss
     WHERE     sc.SOURCE_SYSTEM_OID = ss.SOURCE_SYSTEM_OID
           AND sc.SHIPMENT_OID = s.SHIPMENT_OID
           AND ss.SOURCE_SYSTEM_SHORT_NAME = 'OMS2'
           AND sc.ods_modify_date >
                 TO_DATE (
                     SUBSTR (:v_cdc_load_date, 1, 19),
                     'YYYY-MM-DD HH24:MI:SS')
               - :v_cdc_overlap
           AND NOT EXISTS
                   (SELECT 1
                      FROM rax_app_user.freight_driver_sr t
                     WHERE t.shipment_oid = sc.shipment_oid)
...
MERGE INTO RAX_APP_USER.freight_shipment_detail d
     USING (  SELECT MAX (a.apo_id) AS APO_id, sl.shipment_oid
                FROM ods_own.apo            a,
               ...
               WHERE     a.SUB_PROGRAM_OID = sp.SUB_PROGRAM_OID
                     ...
                     AND oh.ORDER_HEADER_OID = ol.ORDER_HEADER_OID
                     AND ol.ORDER_LINE_OID = sl.ORDER_LINE_OID
                     AND sl.SHIPMENT_OID = dtl.SHIPMENT_OID
                     AND dr.shipment_oid = dtl.shipment_oid
            GROUP BY sl.SHIPMENT_OID) s
        ON (d.shipment_oid = s.shipment_oid)
WHEN MATCHED
THEN
    UPDATE SET d.apo_id = s.apo_id, d.ods_modify_date = SYSDATE
...
/*-----------------------------------------------*/
/* TASK No. 11 */
/* Merge APO Level Attributes */

MERGE INTO RAX_APP_USER.freight_shipment_detail d
     USING (SELECT DISTINCT a.apo_id,
                   a.SCHOOL_YEAR,
                   sp.SUB_PROGRAM_NAME,
                   acct.ACCOUNT_NAME
              FROM ods_own.apo          a,
                   ODS_OWN.SUB_PROGRAM  sp,
                   ods_own.account      acct,
                   RAX_APP_USER.freight_shipment_detail dtl,
                   freight_driver_sr dr
             WHERE     1=1
                   AND a.SUB_PROGRAM_OID = sp.SUB_PROGRAM_OID
                   AND a.ACCOUNT_OID = acct.ACCOUNT_OID
                   AND dtl.apo_id = a.apo_id
                   AND dtl.shipment_oid = dr.shipment_oid
                   AND dtl.ods_modify_date >= TO_DATE(SUBSTR(:v_cdc_load_date,1,19),'YYYY-MM-DD HH24:MI:SS') - :v_cdc_overlap ) s
        ON (d.apo_id = s.apo_id)
WHEN MATCHED
THEN
    UPDATE SET
        ...
        d.ods_modify_date = SYSDATE
             WHERE    DECODE (s.school_year, d.school_year, 1, 0) = 0
                   OR DECODE (s.sub_program_name, d.sub_program_name, 1, 0) = 0
