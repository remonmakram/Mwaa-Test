...
BEGIN  
   EXECUTE IMMEDIATE 'drop table RAX_APP_USER.actuate_seniterrsal_comp_stage';  
EXCEPTION  
   WHEN OTHERS THEN  
      IF SQLCODE != -942 THEN  
         RAISE;  
...
create table RAX_APP_USER.actuate_seniterrsal_comp_stage  as
...
,      sum(tax_amt) tax_amt
,      sum(shipped_Sales_amt) shipped_Sales_amt
,      sum(gross_ship_sales_amt) gross_ship_sales_amt
,      sum(gross_ship_paid_amt) gross_ship_paid_amt
,      sum(Terr_earnings_amt) Terr_earnings_amt
from
